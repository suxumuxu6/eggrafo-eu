...
export const FeaturedDocumentsSection: React.FC<FeaturedDocumentsSectionProps> = ({
  ...
  const featured = featuredDocs.map(item => {
    const match = documents.find(doc => doc.title.trim().toLowerCase() === item.name.trim().toLowerCase());
    return {
      ...item,
      doc: match
    };
  ...
  return (
    <section className="w-full mb-12">
      ...
      {/* The blue square matches the full width of the container */}
            ...
            <div key={name} className="bg-white rounded-xl shadow card-hover p-5 flex flex-col items-center border border-gray-100">
              ...
              <div className="text-center flex flex-col flex-1 w-full">
                <h3
                  ...
                  } text-kb-darkgray mb-3 mx-auto w-full
                ...
                <Button variant="secondary" className="flex items-center gap-2 mx-auto mt-auto" asChild disabled={!doc}>
                  {doc ? <a href={doc.url} download target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
