  ...
  return <>
      <Card className="card-hover card-gradient transition-all duration-200 w-full h-full flex flex-col">
        ...
        <CardFooter className="border-t bg-gray-50/50 p-3 flex gap-2 mt-auto">
          <Button variant="secondary" className="flex-1 text-white bg-kb-blue hover:bg-kb-blue/90 text-xs h-8" onClick={handleViewClick}>Download</Button>
          {isAdmin && <>
              <Button variant="outline" size="sm" onClick={onEdit} className="h-8 w-8 p-0">
                <Edit className="h-3 w-3" />
              </Button>
              <Button variant="outline" size="sm" onClick={onDelete} className="text-red-600 hover:text-red-700 hover:bg-red-50 h-8 w-8 p-0">
      ...
      <DonationModal isOpen={isDonationModalOpen} onClose={() => setIsDonationModalOpen(false)} onSuccess={handleDonationSuccess} documentTitle={title} documentId={id} />
    </>;
};
